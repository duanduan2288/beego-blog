...
<div class="row">
	<div class="col-md-8">
		<form action="http://127.0.0.1:8080/posts/save" method="post" id="form_posts">
			<h3 class="form-title">修改文章</h3>
			<div class="alert alert-danger" style="display:none;" role="alert">{{.flash.error}}</div>
			<div class="form-group">
			   <label for="title">标题</label>
			   <input type="hidden" name="id" value="{{.id}}">
			   <input type="text" class="form-control" value="{{.title}}"name="title" id="title" placeholder="请输入标题">
			</div>
			<div class="form-group">
